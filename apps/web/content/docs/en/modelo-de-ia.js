const content = {
  group: "Configuration",
  title: "AI model",
  lead: "Choose the provider under Settings → AI. The current integration offers local Ollama, the OpenAI API, the Anthropic API, the Google Gemini API, and OpenRouter.",
  blocks: [
    { type: "h2", text: "Ollama" },
    ...
    { type: "para", text: "For local processing, select a local Ollama model; a loopback address alone does not prove where the server runs the model. OpenAI, Anthropic, Google Gemini, and OpenRouter receive the request and chat sample; OpenRouter also forwards them to the model provider. Token totals are partial, not a billing estimate. Canceling does not reverse usage already incurred." }
  ]
};
