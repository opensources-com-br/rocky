...
Esta avaliação é histórica. O escopo atual está em [preparação de lançamento](RELEASE_PREPARATION.md), e os critérios de teste estão no [roteiro de validação](TEST_TODAY.pt-BR.md). Os achados abaixo descrevem o commit analisado, não o estado atual de cada funcionalidade.
