...
Esta avaliação é histórica. As correções posteriores e os critérios de teste do candidato 1.0.10-alpha.4 estão no [roteiro de validação de hoje](TEST_TODAY.pt-BR.md). Os achados abaixo descrevem o commit analisado, não o estado atual de cada funcionalidade.
