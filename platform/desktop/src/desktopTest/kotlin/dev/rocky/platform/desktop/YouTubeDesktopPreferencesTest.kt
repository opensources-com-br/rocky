...
class YouTubeDesktopPreferencesTest {
    @Test fun storesSecretOutsidePreferencesAndClearsIt() {
        ...
        try {
            val store = SecureYouTubePreferences(preferences, secrets)
            store.configuration = YouTubeConfiguration(" client ", " secret ")
            assertNull(preferences.get("clientSecret", null))
            assertEquals("client", store.configuration.clientId)
