...
internal class FacebookAuthorizationReceiver(
    redirectUri: String,
    ...
    private val executor = Executors.newSingleThreadExecutor { task ->
        Thread(task, "rocky-facebook-callback").apply { isDaemon = true }
    }
    private val server: HttpServer

    init {
        require(redirect.scheme == "http" && redirect.host in setOf("localhost", "127.0.0.1")) {
            "O callback do Facebook deve usar um endereço local."
        }
        require(redirect.port in 1..65535) { "O callback do Facebook precisa informar uma porta." }
    ...
    private fun handleCallback(exchange: HttpExchange) {
        ...
        val error = query["error_description"]
        if (query["state"] != expectedState || query["code"].isNullOrBlank()) {
            return exchange.respond(400, error ?: "Autorização inválida. Volte ao Rocky e tente novamente.")
        }
        exchange.respond(200, "Facebook conectado. Você pode fechar esta janela e voltar ao Rocky.")
